...
public class SqlQueryTest extends AcceptanceHarness {
    ...
    public void valid_query_as_payload() throws Exception {
        response =
            client().request(Mime.XML.type()).post(Entity.entity(COUNT_QUERY, Mime.QUERY_SQL.type()));
        assertThat(response, hasFamily(SUCCESSFUL));
        assertThat(response.getMediaType(), is(compatibleTo(Mime.XML.type())));
    }
    ...
    public void delivers_csv() throws Exception {
        response =
            client().request(Mime.CSV.type()).post(Entity.entity(SCAN_QUERY, Mime.QUERY_SQL.type()));
        assertThat(response, hasFamily(SUCCESSFUL));
    ...
    public void delivers_xml() throws Exception {
        response =
            client().request(Mime.XML.type()).post(Entity.entity(SCAN_QUERY, Mime.QUERY_SQL.type()));
        assertThat(response, hasFamily(SUCCESSFUL));
        assertThat(response.getMediaType(), is(compatibleTo(Mime.XML.type())));
    ...
    public void delivers_xml_if_wildcard_accept_header_given() throws Exception {
        response =
            client().request(MediaType.WILDCARD_TYPE).post(Entity.entity(COUNT_QUERY, Mime.QUERY_SQL.type()));
        assertThat(response, hasFamily(SUCCESSFUL));
        assertThat(response.getMediaType(), is(compatibleTo(Mime.XML.type())));
    }

    @Test
    public void redirects_to_meta_schema() throws Exception {
        response = client().path("schema").request().get();
        assertThat(response, hasStatus(Response.Status.MOVED_PERMANENTLY));
        assertThat(response, hasHeader(equalTo(HttpHeaders.LOCATION), equalToIgnoringCase(readAccess().resolve("meta/schema").toString())));
    }
}
