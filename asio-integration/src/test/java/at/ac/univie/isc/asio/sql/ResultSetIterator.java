...
/**
 * Turns a ResultSet into an Iterator using a given {@link RowConverter} to
 * process each row of the ResultSet into an object.
 * 
 * @author Chris Borckholder
 * @param <T>
 *            type of converted rows
 */
public final class ResultSetIterator<T> extends AbstractIterator<T> implements
		AutoCloseable {
	 ...
	 *            will be iterated
	 * @param converter
	 *            converts each row of resultSet to an object
	 * @return the iterator
	 */
	...
	/**
	 * Convert the given {@link ResultSet} to an {@link Iterable} over the
	 * conversion results of the given {@link RowConverter}.
	 * <strong>Caution:</strong>This iterable can only be traversed once!
	 * 
	 ...
	 * @author Chris Borckholder
	 * @param <ROW>
	 *            type of conversion results
	 */
	// @FunctionalInterface - java8
	public static interface RowConverter<ROW> {
		/**
		 * Convert the given ResultSet to an object of type ROW. Should not
		 * modify or advance the given ResultSet. Implementations should
		 * <strong>never</strong> produce null as conversion result.
		 * 
		 * @param resultSet
		 *            set to current row
		 * @return result of conversion (non-null)
		 * @throws SQLException
		 *             occurring while interacting with the ResultSet
		 */
		@Nonnull
	 ...
	 * @author Chris Borckholder
	 */
	public static class ResultSetIterationException extends
	...
	private ResultSetIterator(final ResultSet resultSet,
		...
		this.converter = checkNotNull(converter,
				"cannot use null function as converter");
	}
