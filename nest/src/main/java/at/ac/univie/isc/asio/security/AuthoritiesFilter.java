 ...
 * set of authorities.
 */
public final class AuthoritiesFilter implements GrantedAuthoritiesMapper {
  private final Set<GrantedAuthority> excluded;

  private AuthoritiesFilter(final Iterable<GrantedAuthority> excluded) {
    this.excluded = ImmutableSet.copyOf(excluded);
  }

  public static AuthoritiesFilter exclude(final Iterable<GrantedAuthority> excluded) {
    return new AuthoritiesFilter(excluded);
  }
