...
public class JdbcSpecTest {
  ...
  public void should_build_correct_spec() throws Exception {
    spec = JdbcSpec.connectTo("jdbcUrl").with("jdbcDriver").authenticateAs("user", "password").use(TimeoutSpec.from(1, TimeUnit.SECONDS)).complete();
    ...
    assertThat(spec.getTimeout(), is(TimeoutSpec.from(1L, TimeUnit.SECONDS)));
  }
