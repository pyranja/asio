...
import static at.ac.univie.isc.asio.tool.DatasourceSpec.connectTo;
...
public class DatasourceSpecTest {
  ...
  public void should_guess_jdbc_driver() throws Exception {
    spec = connectTo("jdbc:h2:test").and().noAuthentication();
    assertThat(spec.getJdbcDriver(), is("org.h2.Driver"));
  }

  @Test
  public void should_leave_jdbc_url_as_is_if_no_default_options_necessary() throws Exception {
    spec =
        connectTo("jdbc:genericdb://example.com/test").with("org.example.Driver").authenticateAs(null, null);
  ...
  public void should_append_mysql_default_options() throws Exception {
    spec = connectTo("jdbc:mysql://example.com/test").with("org.example.Driver").noAuthentication();
