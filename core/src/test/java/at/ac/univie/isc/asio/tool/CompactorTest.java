...
public class CompactorTest {
  ...
  private Compactor subject = Compactor.TRIM;
  private String text;
  ...
  public void should_remove_leading_prefixes() throws Exception {
    text =
        String.format(Locale.ENGLISH, "PREFIX ov: <http://open.vocab.org/terms/>%nPREFIX xsd: <http://www.w3.org/2001/XMLSchema#>%nSELECT *%nWHERE { ?s ?p ?o }");
  ...
  public void should_shorten_long_commands() throws Exception {
    ...
    assertThat(text.length(), is(Matchers.greaterThan(Compactor.MAX_LENGTH)));
    assertThat(subject.apply(text).length(), is(Compactor.MAX_LENGTH));
  }
  ...
  public void should_add_truncation_marker_to_shortened_command() throws Exception {
    text = Strings.repeat("my command", 20);
    assertThat(text.length(), is(Matchers.greaterThan(Compactor.MAX_LENGTH)));
    assertThat(subject.apply(text), Matchers.endsWith(Compactor.TRUNCATION_MARKER));
  }
}
