...
/**
 * Inspect request events (status changes and errors) to decide whether to notify the given
 * {@link CompletionCallback} and/or stop tracking.
 * 
...
public class EventAcceptor {
  ...
  public boolean handleStateAndStop(final RequestExecutionStatus status) {
    ...
    if (status.hasFinished()) {
      if (status == COMPLETED) {
        ...
        delegate.fail(new RequestProcessingException(new IllegalStateException("unknown error")));
   ...
   * 
   * @param cause of request failure
   * @return true if the request should not be tracked anymore
   */
