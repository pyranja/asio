...
/**
 * Route filtered OGSADAI request completion and errors to active {@link CompletionCallback}s.
 * 
 * <p>
 * If a {@link CompletionCallback} is registered for a request id, any change in the request's
 * status that signals the termination of the request is inspected and the either the
 * {@link CompletionCallback#complete()} or {@link CompletionCallback#fail(Exception)} method are
 * called. As soon as a request reached a terminal state, it will not be tracked anymore.
 * </p>
 * <p>
 * Additionally, if a request id is setup for tracking before the request is created by OGSADAI,
 * appropriate listeners are attached to it, which also listen for errors in its activities. If any
 * activity encounters an error, it is assumed that the request is failing and
 * {@link CompletionCallback#fail(Exception)} is called. Note that if multiple activities encounter
 * errors, {@link CompletionCallback#fail(Exception)} may be called multiple times, until the
 * request terminates.
 * </p>
 * 
...
public class RequestEventRouter implements MonitoringFramework, RequestListener {
   ...
   */
  @Override
  public void requestErrorEvent(final ResourceID requestId, final DAIException cause) {
    final EventAcceptor acceptor = handlers.get(requestId);
    if (acceptor != null) {
  ...
  public void registerListeners(final RequestDescriptor request,
    ...
    if (handlers.containsKey(requestId)) {
      context.registerActivityListener(new ActivityErrorListener(this, requestId));
    }
  }

  /**
   * Atomically associate the given request id with the given {@link CompletionCallback}.
   * 
   * @param requestId to be tracked
   * @param callback routing target for events with requestId
   * @throws IllegalArgumentException if the given requestId is already tracked
   */
  public void track(final ResourceID requestId, final CompletionCallback callback) {
    ...
    if (former != null) {
      throw new IllegalArgumentException("request [" + requestId + "] already tracked");
    }
  }
   ...
   * 
   * @param requestId that will not be tracked anymore
   * @return the tracker that was associated with the given id, if there was one
   */
  ...
  @Override
  public void newRequestEvent(final ResourceID requestID, final SecurityContext securityContext) {}

  @Override
  public void requestWorkflowEvent(final ResourceID requestID, final Workflow workflow) {}

  @Override
  public void requestPipelineEvent(final ResourceID requestID, final ActivityPipeline pipeline) {}
