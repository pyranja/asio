...
/**
 * Adapt an enclosed {@link BlockWriter} to the {@link OutputStream} interface. Bytes written to
 * this stream are divided into fixed size byte arrays and forwarded to the enclosed writer.
 * 
...
public class BlockWriterStreamAdapter extends OutputStream {
  ...
  BlockWriterStreamAdapter(final BlockWriter writer, final int blockSize) {
    ...
    checkArgument(blockSize > 0, "blocksize must be greater than zero but was %s", blockSize);
  ...
  /**
   * Note that the enclosed {@link BlockWriter} is <strong>not</strong> closed.
   */
  @Override
  ...
  /**
   * Forward the buffer's contents to the set {@link BlockWriter} and prepare the buffer for new
   * writes.
   * 
   * @throws IOException if writing to set BlockWriter fails
   */
   ...
   * @throws IOException wrapping any BlockWriter error
   */
