...
/**
 * Translate Integer blocks to {@link UpdateResult update responses} for the set query and write
 * them as XML document to the given stream.
 * 
...
public class XmlUpdateCountTransformer implements BlockTransformer {
  ...
  public void writeObject(final OutputStream sink, final Object block) throws IOException {
    ...
    final UpdateResult dto = new UpdateResult().withCommand(query).withCount(updateCount);
    serialize(sink, dto);
  }
