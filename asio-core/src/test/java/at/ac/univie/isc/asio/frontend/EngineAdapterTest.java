...
public class EngineAdapterTest {
  ...
  public void completes_op_builder_with_selected_format() throws Exception {
    final OperationBuilder partial = new OperationBuilder("test-id", Action.BATCH, "test-command");
    when(selector.selectFormat(same(request), any(Action.class))).thenReturn(ALWAYS_APPLICABLE);
  ...
  public void sets_context_on_selection_error() throws Exception {
    final OperationBuilder partial = new OperationBuilder("test-id", Action.BATCH, "test-command");
    when(selector.selectFormat(same(request), any(Action.class))).thenThrow(
  ...
  public void submits_op_to_engine() throws Exception {
    subject.submit(op);
    verify(engine).invoke(same(op), any(Transfer.class), any(OperatorCallback.class));
  }

  @Test(timeout = 100)
  public void traps_and_wraps_submission_errors_in_result_future() throws Exception {
    final DatasetOperation op = MockOperations.query("test", ALWAYS_APPLICABLE);
    final Exception error = new IllegalStateException("test");
    doThrow(error).when(engine).invoke(any(DatasetOperation.class), any(Transfer.class),
        any(OperatorCallback.class));
