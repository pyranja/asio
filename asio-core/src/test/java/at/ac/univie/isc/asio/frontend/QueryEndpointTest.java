...
public class QueryEndpointTest extends EndpointTestFixture {
  ...
  public void rejects_unsupported_accept_type() throws Exception {
    client.reset().path("query").query("query", "test-query").accept(MediaType.TEXT_HTML_TYPE);
    response = client.get();
  ...
  public void usage_error_generates_bad_request_response() throws Exception {
    final DatasetUsageException cause = new DatasetUsageException("test-message");
  ...
  public void usage_error_response_contains_cause() throws Exception {
    ...
    when(engine.submit(any(DatasetOperation.class))).thenThrow(cause);
    response = client.query("query", "test-query").get();
    final String received = new String(ByteStreams.toByteArray((InputStream) response.getEntity()));
    assertThat(received, startsWith("[ERROR] " + cause.getMessage()));
  }

  @SuppressWarnings("unchecked")
  @Test
  public void dataset_error_generates_server_error_response() throws Exception {
    when(engine.submit(any(DatasetOperation.class))).thenThrow(DatasetException.class);
    response = client.query("query", "test-query").get();
    assertEquals(Status.INTERNAL_SERVER_ERROR, fromStatusCode(response.getStatus()));
  }
