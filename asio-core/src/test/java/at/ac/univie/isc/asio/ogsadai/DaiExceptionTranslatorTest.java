...
public class DaiExceptionTranslatorTest {
  ...
  @Test
  public void uses_root_cause_as_message_but_preserves_stacktrace() throws Exception {
    final Exception error = new ActivityProcessingException(new IllegalStateException("my error"));
    final DatasetFailureException expected =
        new DatasetFailureException(new IllegalStateException("my error"));
    translated = subject.translate(error);
    assertThat((Exception) translated.getCause(), is(error));
  ...
  public void wraps_user_exceptions_as_usage_exception() throws Exception {
    final Exception error = new ActivityUserException(ErrorID.ACTIVITY_HAS_WRONG_NUMBER_OF_INPUTS);
    translated = subject.translate(error);
