...
public class BlockWriterStreamAdapterTest {
	...
	@Mock private BlockWriter writer;
	@Captor private ArgumentCaptor<byte[]> block;
	...
	@Test
	public void single_bytes_are_written_when_block_size_exceeded()
			throws Exception {
	...
	public void writes_double_and_a_bit_bytes_divided() throws Exception {
		...
		for (final byte[] each : block.getAllValues()) {
			final byte[] expected = Arrays.copyOfRange(data, offset, offset
					+ BLOCK_SIZE);
