...
public class DeliverToStreamTest {
  ...
  @Spy
  private final InputStream source = new ByteArrayInputStream(PAYLOAD);
  @Spy
  private final ByteArrayOutputStream sink = new ByteArrayOutputStream();
  @Mock
  private OutputSupplier<OutputStream> supplier;
  ...
  public void copying_fails() throws Exception {
    ...
    when(mockSource.read(Matchers.<byte[]>any())).thenThrow(new IOException("test-exception"));
    try {
  ...
  private Object[] asArgs(final InputStream stream, final OutputSupplier<OutputStream> supplier) {
