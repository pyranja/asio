...
public class DynamicSerializationActivityTest {
	...
	@Mock BlockTransformer transformer;
	@Mock BlockWriter writer;
	...
	public void invokes_transformer() throws Exception {
		...
		final ArgumentCaptor<OutputStream> stream = ArgumentCaptor
				.forClass(OutputStream.class);
		verify(transformer).writeObject(stream.capture(), same(block));
		assertTrue(stream.getValue() instanceof BlockWriterStreamAdapter);
		assertSame(writer,
				((BlockWriterStreamAdapter) stream.getValue()).getBlockWriter());
	}
	...
	private Object[] asArgs(final Integer count,
