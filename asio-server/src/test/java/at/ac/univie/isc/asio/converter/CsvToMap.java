...
public final class CsvToMap {
	...
	public static Map<String, KeyedRow> convertStream(final InputStream stream,
			final String keyColumn) {
		try (InputStreamReader csv = new InputStreamReader(stream,
				Charsets.UTF_8)) {
	...
	private static int determineKeyIndex(final String[] header,
			final String keyColumn) {
		...
		throw new IllegalStateException("failed to find key column ("
				+ keyColumn + ") index in " + header);
	}
	 ...
	 */
	public Map<String, KeyedRow> convert(final List<String[]> rows,
			final int keyIndex) {
		final ImmutableMap.Builder<String, KeyedRow> partial = ImmutableMap
				.builder();
