...
public final class ResultSetToMap {
	...
	public static Map<String, KeyedRow> convertStream(final InputStream stream,
		...
		try (InputStreamReader xml = new InputStreamReader(stream,
				Charsets.UTF_8)) {
			...
			try (ResultSet rs = parser.getResultSet();) {
				...
				| WebRowSetResultSetParseException e) {
			throw new IllegalStateException(
					"failed to convert webrowset stream", e);
		}
	}

	public Map<String, KeyedRow> convert(final ResultSet rs,
			final String keyColumn) {
		final Builder<String, KeyedRow> partial = ImmutableMap.builder();
		final Iterable<KeyedRow> rows = ResultSetIterator.asIterable(rs,
				new KeyedRowConverter(keyColumn));
