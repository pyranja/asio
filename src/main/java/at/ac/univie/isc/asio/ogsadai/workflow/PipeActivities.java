...
/**
 * Collection of static factory methods for {@link Producer} and {@link Consumer} in an OGSADAI
 * pipeline workflow.
 * 
...
public final class PipeActivities {
  ...
  public static Producer extractSchema(final ResourceID target) {
    final ActivityDescriptor product = new SimpleActivityDescriptor(target, SQL_SCHEMA_ACTIVITY);
  ...
  public static Producer sqlUpdate(final ResourceID target, final String update) {
    final ActivityDescriptor product = new SimpleActivityDescriptor(target, SQL_UPDATE_ACTIVITY);
    product.addInput(new ActivityInputLiteral("expression", update));
    return producer(product, "result");
  }

  // BOTH
  static final ActivityName TUPLE_WEBROWSET_TRANSFORMER_ACTIVITY =
      asName("uk.org.ogsadai.TupleToWebRowSetCharArrays");

  public static ProducerAndConsumer tupleToWebRowSetCharArrays() {
    final ActivityDescriptor product =
        new SimpleActivityDescriptor(TUPLE_WEBROWSET_TRANSFORMER_ACTIVITY);
  ...
  public static ProducerAndConsumer tupleToCsv() {
    final ActivityDescriptor product = new SimpleActivityDescriptor(TUPLE_CSV_TRANSFORMER_ACTIVITY);
  ...
  static final ActivityName TABLEMETADATA_XML_TRANSFORMER_ACTIVITY =
      asName("uk.org.ogsadai.TableMetadataToXMLCharArraysList");

  public static ProducerAndConsumer metadataToXml() {
    final ActivityDescriptor product =
        new SimpleActivityDescriptor(TABLEMETADATA_XML_TRANSFORMER_ACTIVITY);
    return both(product, "data", "result");
  }

  static final ActivityName DYNAMIC_TRANSFORMER_ACTIVITY =
      asName("at.ac.univie.isc.DynamicSerialization");

  public static ProducerAndConsumer dynamicSerializer(final BlockTransformer transformer) {
    final ActivityDescriptor product = new SimpleActivityDescriptor(DYNAMIC_TRANSFORMER_ACTIVITY);
    product.addInput(new ActivityInputLiteral(INPUT_TRANSFORMER, from(transformer)));
    return both(product, "data", "result");
  }

  // CONSUMER
  static final ActivityName REQUESTSTATUS_DELIVERY =
      asName("uk.org.ogsadai.DeliverToRequestStatus");
  ...
  public static Consumer deliverToStream(final OutputSupplier<OutputStream> supplier) {
    final ActivityDescriptor product = new SimpleActivityDescriptor(STREAM_DELIVERY);
    product.addInput(new ActivityInputLiteral(INPUT_SUPPLIER, from(supplier)));
    return consumer(product, "input");
  }
