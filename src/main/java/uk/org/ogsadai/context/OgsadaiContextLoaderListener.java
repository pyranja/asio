...
/**
 * Initialize the {@link OGSADAIContext} on webapp start using the bean
 * definition file specified as context parameter {@link #CONFIG_FILE_PARAMETER
 * ogsadaiConfigLocation}.
 * 
...
public class OgsadaiContextLoaderListener implements ServletContextListener {
	...
	private static final Logger log = Logger
			.getLogger(OgsadaiContextLoaderListener.class.getName());
	...
	public void contextInitialized(final ServletContextEvent event) {
		...
		try {
			...
			throw new IllegalStateException(
					"ogsadai initialization - failed to generate parent context",
					e);
		}
	...
	/**
	 * reads and validates the ogsadai config location parameter from the given
	 * ServletContext.
	 */
	private String readConfigLocation(final ServletContext context) {
		...
		if (location == null || location.trim().isEmpty()) {
			throw new IllegalStateException(
					"ogsadai initialization - init param ['"
							+ CONFIG_FILE_PARAMETER
							+ "'] empty or missing in web.xml");
		}
		return location;
	}

	/**
	 * Generate a parent {@link ApplicationContext} holding the dynamic
	 * references to important application directories.
	 * 
	 * @throws ClassNotFoundException
	 *             if registering directory beans fails
	 */
	private ApplicationContext initializeParent(final File root)
			throws ClassNotFoundException {
		final GenericApplicationContext parent = new GenericApplicationContext();
		SpringUtils.registerFileBean(parent,
				OGSADAIConstants.WEB_APP_DIR.toString(), root);
		final File schemaDirectory = new File(root, SCHEMA_PATH);
		SpringUtils.registerFileBean(parent,
				OGSADAIConstants.SCHEMA_DIR.toString(), schemaDirectory);
		final File configDirectory = new File(root, CONFIG_PATH);
		SpringUtils.registerFileBean(parent,
				OGSADAIConstants.CONFIG_DIR.toString(), configDirectory);
		parent.refresh();
		return parent;
	}
